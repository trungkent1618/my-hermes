...
## Custom Domain State Inconsistency

**Problem**: `POST /accounts/{id}/pages/projects/{project}/domains` returns `8000018: You have already added this custom domain`, but `GET /accounts/{id}/pages/projects/{project}` only lists the `*.pages.dev` domain and the custom domain does not appear in Cloudflare DNS records.

**Cause**: Pages keeps the domain registration internally, but the domain verification/DNS creation did not complete or desynchronized. The API prevents duplicate registration, but the domain is not actually serving traffic.

**Solution**:
1. Confirm the DNS record is missing: `GET /zones/{zone_id}/dns_records?name={subdomain}`
2. If missing, create the CNAME manually:
   ```
   ...
   ```
3. If Pages still rejects re-adding via API, remove and re-add the custom domain from the Cloudflare Dashboard UI to force re-validation.

**Tip**: Always verify both Pages domains list and DNS records in the zone when troubleshooting custom domains.
